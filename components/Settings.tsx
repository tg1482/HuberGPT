...
export const Settings: React.FC<SettingsProps> = ({
    ...
    <div>
      <button
        className={`mt-4 flex cursor-pointer items-center space-x-2 rounded-full border border-zinc-600 px-3 py-1 text-sm hover:opacity-50 ${
          activeGroup === group ? "bg-zinc-600 text-white" : getGroupColor(group)
        }`}
  ...
  const SubGroupButton: React.FC<SubGroupButtonProps> = ({ title, style = null, onClick }) => (
    <button
      className={`mt-2 flex cursor-pointer items-center space-x-2 rounded-full px-3 py-1 text-sm text-white hover:opacity-50 ${
        style ? style : "bg-blue-400"
      }`}
      onClick={onClick}
    >
      {title}
  ...
  const handleAgeGroupChange = (e: React.ChangeEvent<HTMLSelectElement>) => {
    setUserProfile((prevProfile) => ({ ...prevProfile, ageGroup: e.target.value }));
    localStorage.setItem("HGPT_USER_AGE", e.target.value);
  };

  const handleSexChange = (e: React.ChangeEvent<HTMLSelectElement>) => {
    setUserProfile((prevProfile) => ({ ...prevProfile, sex: e.target.value }));
    localStorage.setItem("HGPT_USER_SEX", e.target.value);
  };

  const handleFitnessLevelChange = (e: React.ChangeEvent<HTMLSelectElement>) => {
    setUserProfile((prevProfile) => ({ ...prevProfile, fitnessLevel: e.target.value }));
  ...
  const handleAnythingElseInput = (e: React.ChangeEvent<HTMLInputElement>) => {
    setUserProfile((prevProfile) => ({ ...prevProfile, anythingElse: e.target.value }));
      ...
      {showSettings && (
        <div className="justify-center space-x-2 flex">
              ...
              <div>
                ...
                {userSignedIn
                  ? renderSubGroupContent(
                      "credentials",
                      <div>
                        <SubGroupButton title="Sign Out" onClick={handleSignOut} />
                      </div>
                    )
                  : renderSubGroupContent(
                      "credentials",
                      <div>
                        ...
                        <SubGroupButton title="Sign In" onClick={handleSignIn} />
                      </div>
                    )}
                <SubGroupButton title="Sign Up" style="bg-green-600" onClick={() => handleSubGroupButtonClick("signup")} />
                {renderSubGroupContent(
                  "signup",
                  <div>
                    {renderInputField("email", "Email", (e) => setEmail(e.target.value))}
                    {renderInputField("password", "Password", (e) => setPassword(e.target.value))}
                    <SubGroupButton title="Free 5 Queries" onClick={handleFreeSignUp} />
                    <SubGroupButton title="$5 for 50 Queries" onClick={handlePaidSignUp} />
                  </div>
                )}
                <SubGroupButton title="API Key" style="bg-green-600" onClick={() => handleSubGroupButtonClick("apiKey")} />
                {renderSubGroupContent(
                  "apiKey",
                  <div>
                    {renderInputField("password", "API Key", (e) => setApiKey(e.target.value))}
                    <SubGroupButton title="Submit" onClick={handleApiKeySubmit} />
                  </div>
                )}
              </div>
            ),
          ...
          {renderGroupButton({
            ...
            content: (
              <div>
                <SubGroupButton title="Age Group" style="bg-blue-600" onClick={() => handleSubGroupButtonClick("ageGroup")} />
                {renderSubGroupContent(
                  "ageGroup",
                  <div className="mt-2">
                    <select
                      ...
                      value={userProfile.ageGroup ? userProfile.ageGroup : "25-35"}
                    >
                ...
                <SubGroupButton title="Sex" style="bg-blue-600" onClick={() => handleSubGroupButtonClick("sex")} />
                {renderSubGroupContent(
                  "sex",
                  <div className="mt-2">
                    <select
                      ...
                      value={userProfile.sex ? userProfile.sex : "Female"}
                    >
                ...
                <SubGroupButton title="Fitness Level" style="bg-blue-600" onClick={() => handleSubGroupButtonClick("fitnessLevel")} />
                {renderSubGroupContent(
                  "fitnessLevel",
                  <div className="mt-2">
                    <select
                      ...
                      value={userProfile.fitnessLevel ? userProfile.fitnessLevel : "Moderate"}
                    >
                ...
                <SubGroupButton title="Anything Else" style="bg-blue-600" onClick={() => handleSubGroupButtonClick("anythingElse")} />
                {renderSubGroupContent(
                  "anythingElse",
                  <div>{renderInputField("text", "Additional context", (e) => handleAnythingElseInput(e))}</div>
                )}
              </div>
            ),
          })}
          {renderGroupButton({
            ...
            content: (
              <div>
                <SubGroupButton
                  title="Default"
                  style={userProfile.searchParameters == "Default" ? "bg-red-600" : "bg-red-400"}
                  onClick={() => setUserProfile({ ...userProfile, searchParameters: "Default" })}
                />
                <SubGroupButton
                  title="Protocol"
                  style={userProfile.searchParameters == "Protocol" ? "bg-red-600" : "bg-red-400"}
                  onClick={() => setUserProfile({ ...userProfile, searchParameters: "Protocol" })}
                />
                <SubGroupButton
                  title="Detailed"
                  style={userProfile.searchParameters == "Detail" ? "bg-red-600" : "bg-red-400"}
                  onClick={() => setUserProfile({ ...userProfile, searchParameters: "Detail" })}
                />
              </div>
            ),
          })}
